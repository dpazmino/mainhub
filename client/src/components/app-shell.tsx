...
function SidebarContent({
  ...
  activeRole: { label: string; icon: React.ReactNode; href: string };
  navSections: { label: string; items: { label: string; href: string; icon: React.ReactNode; testId: string }[] }[];
  location: string;
  searchString: string;
  onNavClick?: () => void;
}) {
  return (
    <>
      <div className="flex items-start justify-between gap-3">
        <Link href="/" className="flex items-center gap-3" data-testid="link-sidebar-home" onClick={onNavClick}>
          <div
        ...
        <Badge
          variant="secondary"
          className="rounded-full bg-white/70 border border-border hidden sm:inline-flex"
      ...
      <div className="mt-4 rounded-2xl border border-border bg-white/60 p-3" data-testid="card-sidebar-role">
        <div className="flex items-center justify-between gap-3">
          ...
          >
            <Link href="/" onClick={onNavClick}>Switch</Link>
          </Button>
        </div>
      </div>
...
export function AppShell({
  ...
  return (
    <div className="min-h-screen by-noise" data-testid="layout-app-shell">
      ...
      <div className="relative mx-auto w-full max-w-6xl px-4 md:px-8 py-4 md:py-6">
        {/* Mobile Header */}
        <div className="lg:hidden mb-4">
          <div className="flex items-center justify-between rounded-2xl border border-border/70 bg-white/80 backdrop-blur p-3">
            <Link href="/" className="flex items-center gap-2">
              <div className="grid h-8 w-8 place-items-center rounded-xl border border-border bg-white shadow-sm">
                <span className="font-serif text-sm">BY</span>
              </div>
              <span className="text-sm font-semibold">Better Youth Hub</span>
            </Link>
            <div className="flex items-center gap-2">
              ...
              <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
                ...
                <SheetContent side="left" className="w-[280px] p-4 bg-white/95 backdrop-blur">
                  <SidebarContent
        ...
        <div className="grid gap-4 lg:grid-cols-[280px_1fr] lg:items-start">
          {/* Desktop Sidebar */}
          <aside className="hidden lg:block lg:sticky lg:top-6" data-testid="sidebar-left">
